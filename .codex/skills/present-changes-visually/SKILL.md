...
4. Run the bundled standard-library-only generator from the repository root.
   Use an available Python 3 command. In a Windows Codex desktop workspace,
   load the workspace dependencies and use its bundled Python executable if
   ...
   In PowerShell, use backticks for line continuation or put the command on one
   line. The comparison points can be any Git commit-ish such as `HEAD~1`, a
   tag, a branch, or a commit SHA. Use `WORKTREE` for the current files.
